...
import 'package:tencent_cloud_chat_uikit/ui/views/TIMUIKitConversation/wk_uikit_conversation_last_msg.dart';
...
typedef WKLastMessageBuilder = Widget? Function(
    WKMsg? lastMsg, List<V2TimGroupAtInfo?> groupAtInfoList);

class WKUIKitConversationItem extends TIMUIKitStatelessWidget {
  ...
  Widget _getTimeStringForChatWidget(BuildContext context, TUITheme theme) {
    try {
      if (draftTimestamp != null && draftTimestamp != 0) {
        ...
        return Text(TimeAgo().getTimeStringForChat(lastMsg!.timestamp) ?? "",
            style: TextStyle(
              fontSize: 11,
              color: theme.conversationItemTitmeTextColor,
            ));
